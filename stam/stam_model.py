...
import numpy as np
import base64
from .Letter import image_to_letters, show_img_with_rect, sort_contour, fix_issues_box, sort_contour_one_line
from .rect_util import letter_union, union_many
    ...
    for letter in letters:  # for each contour
        ...
        if letter.status != "OKK":
            cv2.rectangle(img,  # draw rectangle on original training image
                          ...
                          (0, 0, 255) if letter.status in ['wrong', 'superflus']
                          else (255, 0, 0) if letter.status == 'missing' else (0, 255, 0),
                          2)  # thickness
...
def check_image(img_src, f, WEIGHT_FILE):
    ...
    npaContours = get_contour(imgTraining, gshow=False, name=f)
    ...
    print('compare 1')
    x = compare_with_right_paracha(letters)
...
def main():
    src = 'stam/static/images/'
    files = os.listdir(src)
    WEIGHT_FILE = os.listdir('stam/model/output/')[0]  # 'Nadam_beta_1_256_30.hdf5'
    ...
    for num, f in enumerate(files):
        if num == 0:
            ...
            # if not f.contains('result'):
            json_res = check_image(img_src, f, WEIGHT_FILE)
    return json_res
